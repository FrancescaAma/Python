...
# 6
print(cronologia)   # LinkedList([root → admin → mario → guest → sara])

# 7
cronologia.insertBefore("sara", "luca")
# 8
print(cronologia)   # LinkedList([root → admin → mario → guest → luca → sara])

# 9
cronologia.removeFirst()
# 10
print(cronologia)   # LinkedList([admin → mario → guest → luca → sara])

# 11
cronologia.removeLast()
# 12
print(cronologia)   # LinkedList([admin → mario → guest → luca])

# 13
print(f"Modifiche registrate: {cronologia.size()}")

# 14
print(f"Modifica più recente da processare: {cronologia.peekFirst()}")  # admin